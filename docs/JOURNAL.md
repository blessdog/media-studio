# JOURNAL — media-studio, dated history

Append-only. **Not read in full** — this is the archive, not the map. Current
state lives in `STATUS.md` (capped at 150 lines by `tests/test_docs.py`); the
operating manual is `AGENTS.md`.

When STATUS.md trips its line cap, dated sections **move here verbatim**. They
are not deleted. `git log` carries the commit-level record; this file carries
the narrative — what was retracted, what was learned, what was wrong.

Everything below is the former STATUS.md as of the 2026-08-03 split, unedited.
