...
- **[VERIFIED]** External scripting (any process outside Resolve — including every MCP server's default mode) requires **Resolve Studio** ($295 one-time). The free edition restricts scripting to the in-app Console/Scripts menu. Verified against the BMD API README v21.0 (May 2026); re-checked 2026-09-09: the installed 21.1 CHANGELOG only removes Python 2 and adds a built-in interpreter; the press reports the free edition also lost in-app Python scripting [R], forum threads, and three MCP repos. Since 19.1 the restriction has *tightened* (UIManager script GUIs are Studio-only too).
...
| Server | Status | Notes |
|---|---|---|
| **Blackmagic native `ResolveMCP` (Resolve Studio 21.1, 2026-09-08)** | **[VERIFIED live 2026-09-09]** ships inside the app bundle; stdio; 14 tools; smoke-tested against the open Resolve | Design is "code as the tool": `run_script` runs sandboxed Python 3.14 with `resolve`/`project` injected, plus stub/doc/changelog tools and LUT/DCTL authoring. `File > Setup AI Assistants` writes it into Claude Code / Codex / Gemini / Grok configs. Report: `docs/RESOLVE-21.1-AGENTIC.md`. |
| `samuelgursky/davinci-resolve-mcp` | **[VERIFIED live against GitHub API 2026-07-11]** 1,578 ★, v2.60.0 (Jul 6, 2026), actively maintained. **[REPORTED 2026-09-09]** upstream is v2.223.0 and wraps the 21.1 APIs; the copy in `vendor/` is the 2026-07-11 checkout | 34 compound / 341 granular tools; self-reported 336/336 API methods, 331 live-tested. Editing, media pool, render, markers, grading, Fusion, Fairlight helpers. Needs Studio 18.5+, Python 3.10–3.12, macOS OK. **The default choice.** |
...
4. **Dependency fragility**: the best MCP server is one maintainer; Veo versions die on ~9-month cycles (3→3.1 shutdown already happened); Resolve's own scripting now ships Python 3.14 and dropped Python 2 (21.1); the third-party MCP needs 3.10–3.12 and the studio venv is 3.12. Pin versions, wrap vendors behind your own thin interface (SSOT for provider IDs/model names — bible §2.2), expect churn.
