...
| `tools/ingest-osmo.py <clip.MP4>... --name N [--no-compile]` | **the Osmo Action 5 Pro front door** (spec `docs/CINEMATIC-PIPELINE.md` §3-4): originals copied read-only into `outputs/projects/<date>-<name>/media/`, each ffprobed and ASSERTED (hevc, 10-bit, 3840×2160, 24 or 23.976); a failing clip is FLAGGED in `manifest.json` and kept out of the timeline, never imported silently; passing clips become one full-length edit each on V1 of a Story IR, compiled through the normal IR → OTIO → Resolve path, project stamped DaVinci YRGB + Rec.709 / Gamma 2.4 and read back. Exit 2 = BLOCKED on a 23.976 file (lint refuses 1001-denominator rates, VERIFY 12). Writes `report.md`. Applies no grade |
...
| `tools/render-preset.py <name> [--format mp4] [--codec auto] [--bitrate-kbps 80000] [--color-space-tag Rec.709] [--gamma-tag "Gamma 2.4"]` | save or update a named Resolve render preset from the CLI (spec §8): 3840×2160, H.265 Main10 picked from `GetRenderCodecs` when `--codec auto`, bit-rate limit, colour/gamma tags; frame rate follows the timeline. `--gamma-tag` is the one knob the §8 upload test flips. Needs Resolve open with any project current |
